...
Python and its packages (about 120 MB, plus about 150 MB for a browser only if neither
Chrome nor Edge is installed) into ~/.wwtdigital-design. To run it:
    sh "$HERE/setup.sh"
MSG
  exit 3
fi
SCRIPT="$1"; shift
exec "$PY" "$HERE/../skills/wwtdigital-design-system/scripts/$SCRIPT" "$@"
